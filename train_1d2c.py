...
if __name__ == "__main__":
    ...
    # 创建一个全是无信号的train_x
    train_x = np.array([[[0, -110] for i in range(50)] for j in range(len(train_waps_rssis))])
    ...
    train_x = train_x.swapaxes(1, 2)  # none, 52, 2
    ...
    # create an array full of no signal WI-FI for later use
    # 创建一个全是无信号的val_x
    val_x = np.array([[[0, -110] for i in range(52)] for j in range(len(val_waps_rssis))])
    ...
    # reshape from [none, 52, 2] to [none, 52, 2, 1]
    # 把[none, 52, 2] 变成 [none, 52, 2, 1]
    val_x = val_x.swapaxes(1, 2)  # none, 52, 2
    ...
    # add conv
    # 卷积层
    model.add(layers.Conv1D(16, kernel_size=3, activation='relu', input_shape=(train_x.shape[1], train_x.shape[2], train_x.shape[3])))
